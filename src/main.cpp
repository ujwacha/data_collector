...
namespace Serial_Bridge_Skeleton {
	class Sayer : public rclcpp::Node {
		...
		Sayer(const rclcpp::NodeOptions &options)
			: rclcpp::Node("NodeName", options), bridge("/dev/ttyUSB0") {
		...
		void send_data() {
			...
			try {
				bridge.attempt_send_probably_blocking(glosend); // it will always return 1 as it is turned into sendable data already
