...
void print_imu_data(ImuData imu) {
	std::cout << "====IMUDATA====" << std::endl
		...
		<< "pitch: \t" << imu.pitch << std::endl
		<< "roll: \t" << imu.roll << std::endl
		<< "accel_x: \t" << imu.accel_x << std::endl
		<< "accel_y: \t" << imu.accel_y << std::endl
		<< "accel_z: \t" << imu.accel_z << std::endl;
}
...
bool verify_communication(void *d, size_t size) {
	...
	if (((uint8_t*)d)[size - 1] != current_crc) return false;
...
class BridgeClass {
	public:
		...
		bool attempt_send_probably_blocking(Twist_msg d) {
			if (!verify_communication(&d, sizeof(d))) return false;
